@extends('layouts.app')
@section('title', 'Catat Resep Dokter Baru')
...
<script>
    window.prescriptionForm = function() {
        return {
            items: [
                { key: Date.now(), product_id: '', product_name: '', dosage: '', signa: '', quantity: 1 }
            ],
            addItem() {
                this.items.push({
...
<div class="animate-in max-w-6xl mx-auto" x-data="prescriptionForm()">
    <div class="page-header mb-6">
        <div>
            <h2 class="page-title text-2xl font-bold text-gray-800">Catat Resep Dokter Baru</h2>
            <p class="page-subtitle text-gray-500">Mencatat berkas resep yang diterima sebelum dieksekusi di kasir POS</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="lg:col-span-1 card p-6 bg-white border border-gray-100 rounded-2xl shadow-sm h-fit space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="doctor_name" value="{{ old('doctor_name') }}" placeholder="Contoh: dr. Andi Wibowo" class="form-input" required>
                </div>

                <div>
                    <label class="form-label font-bold text-gray-700">SIP Dokter <span class="text-xs text-gray-400">(Opsional)</span></label>
                    <input type="text" name="doctor_sip" value="{{ old('doctor_sip') }}" placeholder="Contoh: SIP.123/2026" class="form-input">
                </div>

                <div>
                    <label class="form-label font-bold text-gray-700">Nama Pasien <span class="text-red-500">*</span></label>
                    <input type="text" name="patient_name" value="{{ old('patient_name') }}" placeholder="Contoh: Ibu Rahmawati" class="form-input" required>
                </div>

                <div>
                    <label class="form-label font-bold text-gray-700">Tanggal Resep <span class="text-red-500">*</span></label>
                    <input type="date" name="prescription_date" value="{{ old('prescription_date', date('Y-m-d')) }}" class="form-input" required>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 card p-6 bg-white border border-gray-100 rounded-2xl shadow-sm flex flex-col">
                ...
                <div class="flex justify-end gap-3 mt-6 border-t border-gray-100 pt-4">
                    <a wire:navigate href="{{ route('prescriptions.index') }}" class="btn btn-secondary">Batal</a>
