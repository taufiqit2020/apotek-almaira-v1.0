...
<script>
    window.purchaseForm = function() {
        return {
            items: {!! json_encode(!empty($prepopulatedItems) ? $prepopulatedItems : [['product_id' => '', 'product_name' => '', 'quantity' => 1, 'unit_name' => '-', 'purchase_price' => 0, 'sell_price' => 0, 'expired_date' => '']]) !!}.map(i => ({
                ...i,
                key: Math.random() + Date.now(),
                purchase_price: Math.round(parseFloat(i.purchase_price) || 0),
                sell_price: Math.round(parseFloat(i.sell_price) || 0),
...
<style>
    ...
    {{-- Qty stepper — konsisten dengan pola qty di halaman Kasir/POS --}}
    ...
    {{-- Baris field angka/harga — flexbox wrap agar TIDAK bergantung pada breakpoint
         viewport, otomatis menyesuaikan lebar kartu (sidebar collapsed/expanded, dsb). --}}
...
<div class="animate-in purchase-create max-w-7xl mx-auto pb-4" x-data="purchaseForm()">
    <div class="page-header mb-5">
        <div>
            <h2 class="page-title text-2xl font-bold text-gray-800">Tambah Barang Masuk</h2>
            <p class="page-subtitle text-gray-500">Isi faktur di atas, lalu tambahkan produk — stok & harga akan diperbarui saat disimpan</p>
        </div>
        <a wire:navigate href="{{ route('purchases.index') }}" class="btn btn-secondary flex items-center gap-2">
    ...
    <form action="{{ route('purchases.store') }}" method="POST" class="space-y-5">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div class="px-5 py-3.5 border-b border-slate-100 bg-slate-50/80 flex items-center justify-between gap-3">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400">Nomor, supplier, tanggal, dan status penerimaan</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-5 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                <div class="sm:col-span-2 xl:col-span-1">
                    ...
                    <input type="text" name="reference_no" value="{{ old('reference_no', $nextRefNo) }}"
                           placeholder="almaira-0001/19/07/2026"
                           class="po-input font-mono font-semibold tracking-tight" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="purchase_date" value="{{ old('purchase_date', date('Y-m-d')) }}" class="po-input" required>
                </div>
                <div>
                    <label class="po-label">Status <span class="text-rose-500">*</span></label>
                    <select name="status" class="po-input" required>
                        <option value="received" {{ old('status', 'received') === 'received' ? 'selected' : '' }}>Diterima (Stok +)</option>
                        <option value="sent" {{ old('status') === 'sent' ? 'selected' : '' }}>Dikirim (PO)</option>
                        <option value="draft" {{ old('status') === 'draft' ? 'selected' : '' }}>Draft</option>
                    </select>
                </div>
                <div class="sm:col-span-2 xl:col-span-4">
                    <label class="po-label">Catatan (opsional)</label>
                    <input type="text" name="notes" value="{{ old('notes') }}" placeholder="Catatan singkat untuk faktur ini…" class="po-input">
                </div>
            </div>
        </section>
                    ...
                    <div class="po-item" :class="item.product_id ? 'is-selected' : ''">
                        <div class="po-item-head">
                            <div class="flex items-center gap-2 min-w-0">
                                <span class="shrink-0 w-7 h-7 rounded-lg bg-slate-100 text-slate-600 text-xs font-extrabold flex items-center justify-center"
                                      x-text="index + 1"></span>
                                <div class="min-w-0">
                        ...
                        {{-- Cari produk — selalu full width, dropdown selalu punya ruang cukup --}}
                        <div class="relative mb-3" x-data="{ openSearch: false, results: [], loading: false, searched: false }">
                            ...
                            <div class="relative" @click.away="openSearch = false">
                                ...
                                <input type="text"
                                       ...
                                       placeholder="Ketik minimal 2 huruf nama / kode produk…"
                                       ...
                                       "
                                       @focus="if (item.product_name.length > 1) openSearch = true"
                                       required>
                        ...
                        {{-- Qty, Satuan, Harga Beli, Harga Jual, Expired — flex-wrap, tidak pernah kepotong --}}
                            ...
                            <div class="po-field-buy">
                                ...
                                <div class="po-money-wrap">
                                    ...
                                    <input type="text" inputmode="numeric" autocomplete="off"
                                           class="po-input po-input-money po-input-with-rp"
        ...
        <div class="po-sticky-bar px-4 sm:px-5 py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                <a wire:navigate href="{{ route('purchases.index') }}" class="btn btn-secondary flex-1 sm:flex-none justify-center">Batal</a>
